      ...
      <div className="flex w-full flex-col gap-[22px]">
        <header className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex max-w-[760px] flex-col items-start gap-2.5">
            <p className="mt-[-1.00px] [font-family:'Inter',Helvetica] text-xs font-bold leading-[normal] tracking-[0.96px] text-[#c3b2df]">
              New roleplayer helper
            </p>
            <h2 className="mt-[-1.00px] [font-family:'Inter',Helvetica] text-[36px] font-black leading-[41.6px] tracking-[-1.60px] text-[#f7f4fb] sm:text-[40px]">
              Skill information
            </h2>
            <p className="pt-0.5 [font-family:'Inter',Helvetica] text-[15px] font-normal leading-[25.5px] tracking-[0] text-[#a296b6]">
          ...
          <div className="inline-flex items-center gap-2 self-start rounded-xl border border-solid border-[#1a1424] bg-[#0b0711b8] px-3.5 py-2 backdrop-blur-[5px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(5px)_brightness(100%)] lg:self-auto">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M11.6667 7.58331C11.6667 10.5 9.62504 11.9583 7.19837 12.8041C7.0713 12.8472 6.93327 12.8451 6.80754 12.7983C4.37504 11.9583 2.33337 10.5 2.33337 7.58331V3.49998C2.33337 3.17803 2.59476 2.91664 2.91671 2.91664C4.08337 2.91664 5.54171 2.21664 6.55671 1.32998C6.81202 1.11185 7.18806 1.11185 7.44337 1.32998C8.46421 2.22248 9.91671 2.91664 11.0834 2.91664C11.4055 2.91664 11.6667 3.17781 11.6667 3.49998V7.58331" stroke="#f7f4fb" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M5.25 6.99998L6.41667 8.16665L8.75 5.83331" stroke="#f7f4fb" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
            <span className="[font-family:'Inter',Helvetica] text-xs font-bold leading-[normal] tracking-[0.48px] text-[#f7f4fb]">
              3 character paths
            </span>
          </div>
        </header>
        <nav
          ...
          className="inline-flex w-fit flex-wrap items-start gap-1.5 rounded-xl border border-solid border-[#1a1424] bg-[#0d0913] px-1 pb-1 pt-1.5"
        >
          {pathTabs.map((tab) => {
        ...
        <div key={animKey} className="skill-anim grid h-fit grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)]">
          <Card className="h-[720px] rounded-lg border border-solid border-[#1a1424] bg-[#060507] shadow-none">
            <CardContent className="flex h-full flex-col gap-4 p-5">
              <div className="flex items-center justify-between gap-3">
                <h3 className="[font-family:'Inter',Helvetica] text-[22px] font-normal leading-[normal] tracking-[-0.66px] text-[#f7f4fb]">
                  {data.treeTitle}
                </h3>
                <Badge className="rounded-xl border border-solid border-[#1a1424] bg-[#0d0913] px-3 py-2 [font-family:'Inter',Helvetica] text-xs font-bold leading-[normal] tracking-[0] text-[#a296b6] hover:bg-[#0d0913]">
                  {data.treeCount}
                </Badge>
              </div>
                ...
                <div className="grid grid-cols-1 gap-3 pb-1">
                  {data.skills.map((skill) => (
                    <article
                      ...
                      className="grid grid-cols-[112px_minmax(0,1fr)] gap-3.5 rounded-md border border-solid border-[#1a1424] bg-[#0d0913] p-4"
                    >
                      <div className="flex flex-col justify-center gap-1">
                        <span className="[font-family:'Inter',Helvetica] text-[11px] font-bold leading-[normal] tracking-[0.88px] text-[#a296b6]">
                          UNLOCK
                        </span>
                        <span className="[font-family:'Inter',Helvetica] text-[28px] font-black leading-[normal] tracking-[-1.12px] text-[#f7f4fb]">
                          {skill.unlock}
                        </span>
                      </div>
                      <div className="flex flex-col items-start gap-2">
                        <h4 className="[font-family:'Inter',Helvetica] text-lg font-normal leading-[normal] tracking-[-0.36px] text-[#f7f4fb]">
                          {skill.title}
                        </h4>
                        <p className="[font-family:'Inter',Helvetica] text-sm font-normal leading-[23.1px] tracking-[0] text-[#a296b6]">
                          {skill.description}
                        </p>
                        <div className="flex flex-wrap items-start gap-2 pt-1">
                          {skill.tags.map((tag) => (
                            <Badge
                              key={`${skill.title}-${tag}`}
                              className="rounded-xl border border-solid border-[#1a1424] bg-[#0b0711] px-2.5 py-[7px] [font-family:'Inter',Helvetica] text-xs font-bold leading-[normal] tracking-[0] text-[#f7f4fb] hover:bg-[#0b0711]"
                            >
                              {tag}
                            </Badge>
                          ))}
              ...
              <ScrollArea className="h-[618px]">
                <div className="flex flex-col gap-4 pr-[9px]">
                  {data.otherSections.map((section) => (
                    <section
                      key={section.title}
                      className="flex flex-col items-start rounded-md border border-solid border-[#1a1424] bg-[#0d0913] p-4"
                    >
                      <p className="[font-family:'Inter',Helvetica] text-xs font-bold leading-[normal] tracking-[0.96px] text-[#c3b2df]">
                        {section.eyebrow}
                      </p>
                      <h4 className="pt-2 [font-family:'Inter',Helvetica] text-xl font-normal leading-[normal] tracking-[-0.60px] text-[#f7f4fb]">
                        {section.title}
                      </h4>
                      <p className="pt-2 [font-family:'Inter',Helvetica] text-sm font-normal leading-[23.1px] tracking-[0] text-[#a296b6]">
                        {section.description}
                      </p>
                      <div className="mt-2.5 flex w-full flex-col gap-2.5">
                        {section.items.map((item) => (
                          <article
                            key={item.title}
                            className="flex flex-col gap-[5.4px] rounded-md border border-solid border-[#1a1424] bg-[#0b0711] px-3.5 py-3"
                          >
                            <h5 className="[font-family:'Inter',Helvetica] text-sm font-bold leading-[normal] tracking-[0] text-[#f7f4fb]">
                              {item.title}
                            </h5>
                            <p className="[font-family:'Inter',Helvetica] text-[13px] font-normal leading-[20.8px] tracking-[0] text-[#a296b6]">
                              {item.description}
                            </p>
                          </article>
                        ))}
                  ...
                  <section className="flex flex-col items-start gap-2 rounded-md border border-solid border-[#1a1424] bg-[#0d0913] p-4">
                    <p className="[font-family:'Inter',Helvetica] text-xs font-bold leading-[normal] tracking-[0.96px] text-[#c3b2df]">
                      Guide note
                    </p>
                    <p className="[font-family:'Inter',Helvetica] text-sm font-normal leading-[23.1px] tracking-[0] text-[#a296b6]">
