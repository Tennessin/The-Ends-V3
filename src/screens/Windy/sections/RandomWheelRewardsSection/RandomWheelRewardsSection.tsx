...
function SlotItem({ item, isDice, isHighlighted, isResult, onClick }: SlotItemProps) {
  ...
  const highlighted = isHighlighted
    ? "border-[#c3b2df] bg-[#c3b2df]/10 scale-[1.04]"
    : isResult
    ? "border-[#c3b2df] bg-[#c3b2df]/08 " + (item ? rarityGlow[item.rarity] ?? "" : "")
    : "border-[#1a1424] bg-[#0b0711]";

  return (
    <div
      className={`${base} ${highlighted} ${onClick ? "cursor-pointer hover:border-[#c3b2df]/50" : ""}`}
      onClick={onClick}
    ...
    >
      {isDice ? (
        ...
        <>
          ...
          <p className="w-full truncate text-center [font-family:'Inter',Helvetica] text-[11px] font-bold text-[#f7f4fb]">
            {item.name}
          </p>
          <span className="[font-family:'Inter',Helvetica] text-[10px] font-normal text-[#a296b6]">
            {item.rarity}
          </span>
        </>
      ) : (
        <div className="h-14 w-full animate-pulse rounded-md bg-[#1a1424]" />
      )}
    </div>
  );
}
  ...
  return (
    <section className="relative w-full">
      ...
      {!isDrugMode && dropResults.length > 0 && (
        <div className="mt-6 px-4">
          <div className="rounded-xl border border-[#1c1c1c] bg-[#0b0711] p-4">
            <h3 className="mb-4 [font-family:'Inter',Helvetica] text-[11px] font-bold uppercase tracking-[1.2px] text-[#c3b2df]">
              Your Drop Results
            </h3>
